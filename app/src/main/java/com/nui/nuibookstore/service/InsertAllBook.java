...
public class InsertAllBook extends AsyncTask<Void,Void,Boolean> {
       Context context;
   public InsertAllBook(Context context){
    ...
    protected Boolean doInBackground(Void... voids) {
        ...
        List<Book> books = setAllBook();
        for (int i = 0; i < books.size(); i++) {
            bookDao.insertBook(books.get(i));
        }
//        bookDao.insertBook(new Book("son test again","son","son","novel",12.4,R.drawable.the_transcriptionist));
        return true;
    }
    ...
    private static List<Book> setAllBook(){
